function [Lc, Mc, G, P, K, Cpt, errorHis, time] = algebraicCoarsening(L, M, numNc, varargin)
%%
% Algebraic Coarsening
% "Spectral coarsening of geometric operators" [Liu et al. 2019]
% https://github.com/HTDerekLiu/SpecCoarsen_MATLAB/blob/master/utils/algebraicCoarsening.m
%%
% default parameter values
lr = 2e-2; % learning rate (step size) for the gradient descent
lrDecayIter = 4; % learning rate decay iterations (optional)
    ...
    if objValOld < objVal
        ...
        if stop > stallIter
            if lrDecayCount < lrDecayIter % not decrease lr yet
                ...
                fprintf('decrease learning rate to: %f\n', lr);
            else
                fprintf('iter %i, cost %.4e\n', iter, objValOld);
                break;
            end
